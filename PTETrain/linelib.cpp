...
void line_node::outputSimilarity(char*file_name, int binary)
{
}
...
void line_node::output(const char *file_name, int binary)
{
	...
	for (int a = 0; a != node_size; a++)
	{
		...
		//else 
			for (int b = 0; b != vector_size; b++) fprintf(fo, "%lf ", _vec[a * vector_size + b]);
		fprintf(fo, "\n");
	}
	fclose(fo);
}
 ...
 */
void line_hin::init(const char *file_name, line_node *p_u, line_node *p_v)
{
...
void line_trainer::train_sample(real alpha, real *_error_vec, std::function<double()> func_rand_num, unsigned long long &rand_index)
{
