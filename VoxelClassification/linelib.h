...
public:
	line_node();
	~line_node();
	void outputSimilarity(char *file_name, int binary = 0);
...
public:
	...
	void init(const char *file_name, line_node *p_u, line_node *p_v);
};
...
public:
	...
	void train_sample(real alpha, real *_error_vec, std::function<double()> func_rand_num, unsigned long long &rand_index);
};
